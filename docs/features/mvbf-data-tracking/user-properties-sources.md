...
| `device type` | `ifp / desktop / laptop / tablet / phone` | `_deviceType()`（set 點 `amplitude_user_properties.dart:37`、函式 L157-173） | ✅ 採 A 維持現狀 | IFP → `ifp`；mac/windows → `desktop`；iOS/Android → `tablet`；其他 → `na`。比 cs 完整（cs 幾乎不送，prod 唯一出現過的值是 sparrow 的 `desktop`）。兩點待問 Zoe：不分 `phone`、未知用 `na`（cs 是 omit）。見 `open-questions.md#Q14` |
