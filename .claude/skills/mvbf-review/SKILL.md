...
### 寫完鏈之後，找最便宜的那一段切斷 —— 不一定是根因

決定「知道但先不修」之前，把鏈的**每一步**都看一次能不能便宜地切斷。
最貴的那一步（根因）常常不是唯一的切點，而中間往往有個
「第一次失敗就放棄」的 fallback，改它便宜得多、風險也小得多。

沿用上面那條鏈：根因的解法是取消執行中的工作，但那會在對方可能正在寫入時中斷它，
要實機驗證才敢換。而鏈的中間是**「第一次失敗就退回空快取」**——把它改成短暫重試
（對方持有鎖只有毫秒級），機率就降一個數量級，且沒有中斷寫入的風險。
根因的解法仍記在註解裡當完整解。
...
⚠️ **不只驗對方的診斷，連對方提的修法也要驗算。**「診斷對、修法錯」是常見組合。
實例：reviewer 正確指出某個守衛外溢到了不相關的行為，但他提的替代條件式
會把另一條路（正式版的路徑）也一起改掉 —— 正是他想避免的那種外溢。
照抄會製造一個比原本更大的問題，而且是**他和我都以為在收斂範圍**的情況下發生。
判準很單純：**把兩個版本的條件式各代入所有輸入組合算一次**，不要用讀的。
